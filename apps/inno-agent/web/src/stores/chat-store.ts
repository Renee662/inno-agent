...
import { streamChat, abortChat, streamSessionEvents, getChatStatus, getPendingQuestionForSession } from "../api/chat.js";
import type { InlineImage } from "../api/chat.js";
import type { ChatMessage, ChatStreamEvent, ChatToolRecord, PendingQuestion, PendingQuestionItem, QuestionnaireResult } from "../types/chat.js";
...
class ChatStoreImpl extends EventEmitter<ChatStoreEvents> {
	...
	pendingQuestionItems: PendingQuestionItem[] = [];
	...
	private _handleStreamEvent(event: ChatStreamEvent) {
		switch (event.type) {
			...
			case "question":
				...
				this.emit("change", undefined);
				void this.refreshPendingQuestions();
				break;
			case "done":
	...
	async refreshPendingQuestions(): Promise<void> {
		try {
			const status = await getChatStatus();
			this.pendingQuestionItems = status.pendingQuestions;
			this.emit("change", undefined);
		} catch {
			// keep the previous list on transient API errors
		}
	}

	async loadPendingQuestionForSession(sessionId: string): Promise<void> {
		try {
			this.pendingQuestion = await getPendingQuestionForSession(sessionId);
			this.emit("change", undefined);
			void this.refreshPendingQuestions();
		} catch {
			// no pending question or status endpoint unavailable
		}
	}
